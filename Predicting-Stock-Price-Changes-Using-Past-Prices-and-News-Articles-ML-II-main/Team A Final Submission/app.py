...
def main():
    ...
    st.title("Real-Time Stocks Movement Prediction")
    ...
    if(choose_stock == "Apple"):
        ...
        symbol = 'AAPL'
        startdate = (datetime.datetime.now() - datetime.timedelta(days=3*365)).strftime('%Y-%m-%d')
        enddate = datetime.date.today()

        df1 = yf.download(symbol, start=startdate, end=enddate)

        st.header("Apple Stock Last 5 Days Data:")
        ...
        text_vect = open("tfidfvect.pkl","rb")
        tfvectorizer = joblib.load(text_vect)

        rfc = open("rfc.pkl", "rb")
        rf_classifier = joblib.load(rfc)

        clean_headlines = []
        for daily_headlines in text:
            clean_headlines.append(clean_text(daily_headlines))
        latest_news = ''.join(clean_headlines)
        latest_news = lemmatize_data(latest_news, lemmatizer)
        latest_news_vect = tfvectorizer.transform([latest_news]).toarray()
        pred = rf_classifier.predict(latest_news_vect)
        next_trade_day = datetime.datetime.strptime(date[0],'%Y-%m-%d').date()+datetime.timedelta(days=1) 
        next_trade_day = weekdaychecker(next_trade_day)
        st.subheader('The stock movement on next trading day: ' + str(next_trade_day))
        percnt_change = ((new_preds[0][0]-close_col.iloc[-20])/close_col.iloc[-20])[0]*100
        if pred[0] == 1:
            st.write("Up with percentage change from the past 20th day by: ",percnt_change," %")
        else:
            st.write("Down with percentage change from the past 20th day by: ",percnt_change," %")
